	...
	$comment_counter = 0;
		...
		foreach($view->style_plugin->rendered_fields as $key => $rendered):
				...
				if ($cid == 0):
					...
					$content .= '<a href="'.$base_url.'/node/'.$nid.'">'.$term_name.'</a></td><td>'.$date.'</td><td>(tag)</td>';
					$content .= '</tr>';
					$tag_counter++;
				else:
					$comment_subject= db_result(db_query('SELECT subject FROM {comments} WHERE cid = %d', $cid));
					$content .= '<tr class="ec-note-active"><td class="ec-note-date">';
		...
		/*print '<ol class="user_data_sorting">';
			print '<li><a href="'.$base_url.'/ecomma_user_data/'.arg(1).'/'.arg(2).'/nd">Sort by node</a></li>';
			print '<li><a href="'.$base_url.'/ecomma_user_data/'.arg(1).'/'.arg(2).'/dt">Sort by date</a></li>';
		print '</ol>';*/
		...
		print '<tr class="user_data_sorting">';
			...
			print '<td></td>';
			//print '<td><a href="'.$base_url.'/ecomma_user_data/'.arg(1).'/'.arg(2).'/dt">Sort by type</a></td>';
		print '</tr>';
		print $content;
